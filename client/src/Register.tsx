...
const Register = () => {
  ...
  const handleRegister = async (event: React.FormEvent) => {
    ...
    if (!firstName || firstName.length <= 1) {
      ...
      try {
        ...
        const response = await fetch("https://mak-self-development-4ifpjeej2-maktor.vercel.app/register", {method: "POST",headers: {"Content-Type": "application/json"},body: JSON.stringify({ firstName, username, age, email, password })});
        if (response.ok) {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch("https://mak-self-development-4ifpjeej2-maktor.vercel.app/login", {method: "POST",headers: {"Content-Type": "application/json"},body: JSON.stringify({ username, password })});
